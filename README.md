...
`none` still disables forwarding entirely. The auto-detected cwd (no variable set) keeps its lenient behaviour and never fails the bridge: if it is unreadable, forwarding is silently disabled; if it is not valid UTF-8 — which JSON encoding would corrupt into a phantom project name — forwarding is disabled *and* a warning goes to stderr. Either way the daemon's own directory takes over.
