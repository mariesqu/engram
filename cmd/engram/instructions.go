...
Open with mem_current_project. Three fields decide what you do next:
  ...
  writes_blocked=true  — reads answer from the basename, but mem_save/mem_save_prompt/mem_session_start/mem_session_summary will REFUSE until you pass project explicitly (ambiguous directory, malformed .engram/config.json, missing directory, a RELATIVE directory, no directory at all, or an "omitted" project); a NON-STRING directory is refused even with project — send it as a string or omit it.
